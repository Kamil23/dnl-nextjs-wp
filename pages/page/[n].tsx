...
export default function HomePage({ posts, page, totalPages, menu, preview }) {
  const menuItems = menu?.menuItems?.edges
  // Yoast pattern on the live site: "Dieta na luzie - Strona 2 z 9 - Jak jeść i nie zwariować?"
...
export const getStaticProps: GetStaticProps = async ({ params, preview = false }) => {
  ...
  const totalPages = Math.ceil(allPosts.edges.length / HOME_POSTS_PER_PAGE)
  const posts = allPosts.edges.slice(
    (page - 1) * HOME_POSTS_PER_PAGE,
    page * HOME_POSTS_PER_PAGE
  )
  ...
  return {
    props: { posts, page, totalPages, menu, preview },
    revalidate: 10,
  }
}
